...
.em_password li:first-child {
  width: 70%;
}
.em_password li:last-child {
  width: 30%;
  text-align: right;
  font-size: 0.28rem;
  color: #fff;
}
.em_password li input {
  font-size: 0.28rem;
  color: #fff;
  border: none;
  width: 100%;
}
.em_password li input::-webkit-input-placeholder {
  color: #fff;
}
.em_password li input:-moz-placeholder {
  color: #fff;
}
.em_password li input::-moz-placeholder {
  color: #fff;
...
export default {
  ...
  created: function() {
    ...
    //       var item = new Object();
    //       item.name = this.dataList.list[i].name;
    //       item.value = this.dataList.list[i].hotelId;
    //       this.propertyList.push(item); 
    //       alert(this.propertyList);
    //     }
    //   }
    });
  },
      ...
      if (this.lname && this.pwd) { 
        ...
        this.$store.dispatch("loginStaff", params).then(res => {
          if (res.code == 0) {
            ...
            this.$dialog.toast({
              mes: _this.language.msg.login_err,
              timeout: 1000
            });
          }
        });
