			...
			<scroller ref="scroller">
				<section class="index_top" :style="{backgroundImage: 'url(' + homeData.indexBackground + ')'}">
					...
					<div class="smart_home">
						<button type="button" @click="goService()"><img style="width: .4rem;vertical-align: middle;display: inline-block;" src="../../assets/images/service-btn.png" alt="">{{shortcutList[0].title}}</button>
					</div>
       ...
       <section class="index_content">
                    <div class="swiper-container">
                        <div class="swiper-wrapper">
                            <div class="swiper-slide" v-for="(item,index) in shortcutList" @click="goNext(item.key)" v-show="index != 0">
                                <img :src="item.imgSrc">
                                <p>{{item.title}}</p>
                            </div>
                        </div>
							...
							<div class="swiper-wrapper">
								<div class="swiper-slide" v-for="(item, index) in hotelDetail.roomTypeList">
									...
									<ul class="apart_style">
										<li class="col-5">
											<i></i>{{language.apartment.maxPerson}} {{item.personcount}} {{language.apartment.person}}(s)</li>
										<li class="col-5">
											<i></i>{{item.bedtype}}</li>
									</ul>
									<ul class="apart_util">
		...
		<yd-actionsheet :items="myItems" v-model="tabLanShow" cancel="Cancel"></yd-actionsheet>
		<!--底部菜单、个人中心、 设置项-->
		<v-foot></v-foot>
...
export default {
  ...
  created: function() {
    ...
    this.localshortcutList = [
      ...
      //暂时取消ssr
      // {key: "news", title: '', imgSrc: require('../../assets/images/icon-smart.png'), linkTo: '/ssr'},
      {
        key: "invoice",
        title: "",
    ...
    this.$store.dispatch("getHome", params).then(res => {
      if (res.code == 0) {
        ...
        console.log(this.home.data.shortcutList);
        //整理按钮结果集
    ...
    this.$store.dispatch("getHotelDetail", params2).then(res => {
      ...
      $(document).ready(function() { 
        setTimeout(function() {
          ...
          var swiper1 = new Swiper(".index_content .swiper-container", {
            pagination: ".swiper-pagination",
            slidesPerView: 4,
            paginationClickable: true,
            spaceBetween: 0
...
.smart_home i {
  ...
  width: 0.3rem;
  height: 0.4rem;
  background: url("../../assets/images/icon-smart.png")center no-repeat;
