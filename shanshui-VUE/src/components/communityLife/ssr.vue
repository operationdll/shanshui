<template>
  <div class="ssr">
      ...
      <yd-navbar :title="language.community.ssr" fixed>
          ...
          <span slot="right" @click="show = true">订阅</span>
      </yd-navbar>
      <section class="promotiom_index" style="margin-top: 1.3rem;padding: .1rem 0;position:relative;z-index: 9999;background: #fff;">
          <div class="swiper-container">
              <div class="swiper-wrapper tab-change">
                  <div class="swiper-slide" :class="index ==0 ? 'active': ''" v-for="(item,index) in ssrList" v-show="item.active == 1 ? true : false">
      ...
      <yd-popup v-model="show" position="bottom" height="90%">
         <div class="s-show">
             <h4>我的频道 <span class="s-save" @click="setActive">保存</span></h4>
             <h5 style="margin-top: 20px;">新闻</h5>
             <ul class="ssrList">
	...
	export default {
        ...
        created:function () {
            ...
            this.$store.dispatch('getRss', params).then(function (res) {
                ...
                // 获取选中状态
                var activeList = localStorage.activeList
                if(activeList){
                    // 重组列表数据
                ...
                //初始化页面值
                _this.changeTab(_this.ssrList[0].ssr,_this.ssrList[0].name)

            })

            console.log('fg',this.ssrList)
        },
		methods: {
			getTab:function (arr) {
				this.tabList=[];
            ...
            changeTab:function (url,name) {
				...
				let _this = this;
			    var urlPath;
			    if(name == '腾讯国内新闻' || name == 'news') {
                    urlPath = 'http://lixin.xapi.wangtiansoft.com/newsgn/rss_newsgn.xml'
                } else if(name == '腾讯财经' || name == 'caijing') {
                    urlPath = 'http://lixin.xapi.wangtiansoft.com/scroll/rss_scroll.xml'
                } else if(name == '搜狐娱乐' || name == 'yule') {
                    urlPath = 'http://lixin.xapi.wangtiansoft.com/rss/yule.xml'
                }else {
                    urlPath = ssrBase + url
                }
				//通过jq的get方法获取数据源
